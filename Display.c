...
void draw_input_box(FONT* font, float x, float y, float w, float h, char* nickname) {
    ALLEGRO_COLOR background_color = al_map_rgb(245, 245, 245); // 입력창 배경색
    ALLEGRO_COLOR side_color = al_map_rgb(80, 140, 220);
...
void draw_enemy(GameState* g, IMAGE* i) {
    ...
    for (int j = 0; j < enemies->now_enemy; j++)
    {
        ...
        if (enemies->enemy[j].type == 0) {// 근접일 경우
            ...
            else al_draw_bitmap(i->easy_enemy, enemies->enemy[j].x, enemies->enemy[j].y, 0);
        }
...
void draw_scene_change(GameState* g, FONT* f, Scene* scene, IMAGE* i) {
    ...
    case STAGE2:
        ...
        al_draw_text(f->font_title, al_map_rgb(0, 0, 0), SCREEN_W / 2, SCREEN_H / 2,
...
void draw_boss_hp(GameState* g) {
    ...
    al_draw_filled_rectangle(g->boss.x + 10, g->boss.y - 40, g->boss.x + (int)(185 / 10 * (g->boss.hp)), g->boss.y - 40 - 30, al_map_rgb(255, 0, 0));
}