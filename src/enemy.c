...
int Max_enemies[4] = { 10, 20, 30, 500 };
...
void Create_enemy(GameState* g) { //적 생성
	...
	if (em->spawn_cnt == 0 && em->now_enemy < em->max_enemy) {
		...
		do {
			...
			i_x = Random_f(PLAYER_WIDE, SCREEN_W - PLAYER_WIDE);
			i_y = Random_f(PLAYER_HEIGHT, GROUND_Y - PLAYER_HEIGHT);


			for (int i = 0; i < MAX_OBJECTS; i++) { //모든 오브젝트와 겹칩이 없을떄 까지

				map* obj = &(g->stage_map.objects[i]);

				if (i_x < obj->x + obj->w &&
					i_x + PLAYER_WIDE > obj->x &&
					i_y < obj->y + obj->h &&
					i_y + PLAYER_HEIGHT > obj->y)
				{
		...
		if (!e->spawn)
		{
			if (e->type) {
				...
				if (e->shotTimer == 0) {
					...
					//-----------------------------------------------------
					al_play_sample(sound_paper, 1.0, 0, 1.0, ALLEGRO_PLAYMODE_ONCE, NULL);
					//-----------------------------------------------------
				}
				else e->shotTimer--;
